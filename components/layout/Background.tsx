...
const Background: React.FC<BackgroundProps> = ({ wallpaperUrl, minimalOverlay = false }) => {
  ...
  }, [displayedWallpaper, minimalOverlay]);
  ...
  return (
    <div
      ...
      style={{ zIndex: minimalOverlay ? 0 : -10 }}
    >
      {/* Base gradient background - uses theme colors */}
      <div
        className="absolute inset-0 bg-gradient-to-br"
        style={{
          backgroundImage: minimalOverlay
            ? 'none'
            : 'linear-gradient(to bottom right, var(--theme-bg), var(--theme-surface), var(--theme-bg))',
          backgroundColor: minimalOverlay ? 'transparent' : undefined
        }}>
        {/* Theme-aware wallpaper layer with enhancements - only shown if not NONE */}
        {showWallpaper && wallpaperSrc && (
          <div
            className="absolute inset-0"
            style={{
              opacity: minimalOverlay ? 1 : (imageLoaded ? 0.85 : 0),
              transition: minimalOverlay ? 'none' : 'opacity 300ms ease'
          ...
          >
            <Image
              ...
              fill
              loading={minimalOverlay ? "eager" : "lazy"}
              priority={minimalOverlay}
              quality={85}
              sizes="100vw"
              onLoad={() => {
                if (!minimalOverlay) {
        ...
        {!minimalOverlay && <div className="background-orbs">
        ...
        {/* Vignette effect */}
        {!minimalOverlay && <div className="absolute inset-0 bg-gradient-radial from-transparent via-transparent to-[#1a1b26]/50"></div>}
      </div>

      {/* Stars/particles effect */}
      {!minimalOverlay && <div className="absolute inset-0">
